...
void setSequenceStop(char s2[], char first, char last, int idx){
    s2[idx] = first;
    s2[idx + 1] = '-';
    s2[idx + 2] = last;
}
...
int fillOutput(char s1[], char s2[], int len){
    ...
    for (i=0; i<len; i++) {
        ...
        if ((sequential > 1) && isSpace == 0){
            setSequenceStop(s2, s1[lastIdx], curr, outPutIdx);
            outPutIdx += 3;
            lastIdx = i+1;
            sequential = 0;
            continue;
